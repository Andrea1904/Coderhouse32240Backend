...
export default class Routers {
  ...
  handlePolices = policies => (req, res, next) => {
    ...
    if (!authoHeaders) return  res.status(401).send({ status: "error", error: "No esta Autorizado" });
    const token = authoHeaders.split(" ")[1];
    let user = jwt.verify(token, "coderTest");
    if (!policies.includes(user.role.toUpperCase())) return   res.status(403).send({ error: "No esta Autorizado" });
    req.user=user;
    next();
  };
